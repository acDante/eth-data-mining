...
def reducer(key, values):
    ...
    # values: flattened list of coresets, effectively the union of the coresets
    coreset = values
    ...
    for e in range(epochs):
        ...
        point_count = [[0 for i in range(k)] for r in range(restarts)] # number of points closest to each cenert for each restart
        for x,w in coreset:
